...
티켓 1 장 남았는데 50 명이 _동시에_ 결제 버튼 누름.

- _락 없음_ → 50 명 모두 "결제 성공" 응답 → 티켓 -49 장 (음수)
- _비관 락_ → 1 명만 성공, 49 명은 대기 → 정확
- _낙관 락_ → 모두 시도 → 1 명만 성공, 49 명 재시도 응답

> 부트캠프 5주차 = 학생들이 _가장 많이 무너지는_ 구간. 흥미롭게 가자.
