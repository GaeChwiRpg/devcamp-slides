...
4. Discord · 제출 시스템 흐름
5. 첫 주 시작 신호
6. **팀 결성 · 스터디 합의 · 네트워킹** (오늘 후반 60분)

> _90분 발표 + 60분 액션_. 막히는 곳은 다음 주 토 14:00 에 다시.
...
각 팀 따로 모입니다. 운영진이 팀별 자리 안내.

**한 사람당 90초** — 시간 박스 엄격:

- 이름 / 닉네임 / 현재 직무 (혹은 전공)
- 백엔드 시작한 _계기_ 1줄
- 본인의 _강점_ 1개 + 보완하고 싶은 _약점_ 1개
- 10주 후 _되고 싶은 모습_ 1줄

> 팀장은 _자율 합의_ — 강제 X. 팀 채널 운영을 _가장 먼저_ 챙길 사람이면 OK.
...
팀 전용 채널 (`{기수}-team-NN`) 에 _고정 메시지_ 로 박아둘 4가지:

| 항목 | 합의할 것 |
| --- | --- |
| 질문 올리는 방식 | 채널/스레드 · 시도한 것 1줄 포함 여부 |
| 답변 템포 | 몇 시간 내 답변? (예: 평일 4h / 주말 next-day) |
| PR 리뷰 요청 | `@everyone` vs 지정 · 마감 시각 명시 여부 |
| 일정 밀릴 때 | 며칠 전 공유 · 대체 제출 방식 · 팀장 booster |

> 이게 _없으면_ 9주차 팀 프로젝트에서 무너집니다. _지금_ 정하기.
...
[15분]  팀 자기소개
[15분]  팀 스터디 4가지 합의
[20분]  식사 + 자유 대화 — 팀 外 학생 / 운영진 짝수 인사
[10분]  온라인 운영 채널 연결 확인
        - Discord 본인 학생 채널 입장 확인
        - GitHub 본인 학생 레포 접근 확인
        - 02-week1-spring-boot/README 한 번 훑기
```

> 1기 동기 = 10주 + 그 뒤 _취업·이직 동안 든든한 네트워크_. 오늘 한 명이라도 더.
